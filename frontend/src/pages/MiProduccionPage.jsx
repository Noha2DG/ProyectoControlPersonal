...
// (2 s) porque ahí la persona solo confirma que marcó, y aquí tiene que leer una tabla parada.
...
// límites del clamp evitan que en una ventana muy baja el texto quede ilegible.
const T = {
  titulo:  "text-[clamp(0.85rem,2.4vh,1.4rem)]",
  reloj:   "text-[clamp(1.1rem,3.4vh,2rem)]",
  nombre:  "text-[clamp(1rem,3.6vh,2.4rem)]",
  hero:    "text-[clamp(1.5rem,6.5vh,3.75rem)]",
  tarjeta: "text-[clamp(1rem,4vh,2.4rem)]",
  tabla:   "text-[clamp(0.8rem,2.3vh,1.4rem)]",
  medio:   "text-[clamp(0.8rem,2.2vh,1.35rem)]",
  chico:   "text-[clamp(0.6rem,1.5vh,0.85rem)]",
  campo:   "text-[clamp(1.1rem,4vh,2.4rem)]",
};
...
// fijo y en una ventana baja (o en un monitor 4:3) la tabla se quedaba sin altura y solo se veía el
...
function Identidad({ empleado, puesto }) {
  ...
  return (
    <div className="flex items-center gap-6 bg-slate-900 border border-slate-800 rounded-xl px-6 py-2 min-h-0 overflow-hidden">
      <div className="flex-1 min-w-0">
        <p className={`${T.nombre} font-extrabold text-white leading-tight uppercase truncate`}>{empleado.Nombre}</p>
        <div className="flex items-center gap-3 mt-1">
          <span className={`${T.medio} font-mono text-slate-300 tracking-widest`}>{empleado.Codigo}</span>
          {empleado.Area && (
            <span className={`${T.medio} px-3 py-0.5 rounded-full border border-emerald-700 bg-emerald-500/10 text-emerald-300 font-semibold tracking-widest uppercase`}>
              {empleado.Area}
            </span>
          )}
        </div>
      ...
      {puesto && (
        <div className="shrink-0 flex items-center gap-4 border-l border-slate-800 pl-6">
          <div className="text-right">
            <p className={`${T.medio} font-semibold uppercase tracking-widest ${primero ? "text-amber-300" : "text-slate-200"}`}>
              {primero ? "¡Primer lugar!" : "Puesto del día"}
            </p>
            <p className={`${T.medio} text-slate-400`}>de {puesto.DeCuantos} hoy</p>
          </div>
          <div className="relative shrink-0 w-[clamp(2rem,8vh,4.5rem)] h-[clamp(2rem,8vh,4.5rem)]">
            ...
            <div className={`relative w-full h-full rounded-full border-4 flex items-center justify-center ${
              primero ? "border-amber-400 bg-amber-400/10" : "border-emerald-500"
            }`}>
              <span className={`${T.tarjeta} font-extrabold tabular-nums leading-none ${primero ? "text-amber-300" : "text-white"}`}>
                {puesto.Puesto}
              </span>
            </div>
          </div>
...
function Resultado({ datos }) {
  ...
  // Proporciones fijas en vez de "identidad y totales toman lo que necesiten y la tabla se queda con
  // las sobras": con ese reparto, en una ventana baja a la tabla no le quedaba ni una fila. Así la
  // tabla siempre se lleva algo más de la mitad del alto, sea cual sea la pantalla.
  return (
    <div className="flex-1 min-h-0 grid grid-rows-[minmax(0,1fr)_minmax(0,1.3fr)_minmax(0,4.2fr)] gap-3">
      ...
      {/* El total ocupa el ancho grande y las dos áreas se apilan a su derecha: en horizontal
          cabe todo en una sola franja, sin robarle altura a la tabla. min-h-0 para que en una
          ventana baja esta franja ceda alto en vez de empujar la tabla fuera de la pantalla. */}
      <div className="grid grid-cols-[3fr_2fr] gap-3 min-h-0 overflow-hidden">
        <div className="bg-slate-900 border border-slate-800 rounded-xl flex flex-col items-center justify-center py-1 min-h-0 overflow-hidden">
          <p className={`${T.medio} text-slate-200 font-semibold uppercase tracking-[0.2em]`}>Total procesado hoy</p>
          <p className={`${T.hero} font-mono font-extrabold text-white tabular-nums leading-none mt-1`}>
            {lb(totalKg).toFixed(1)}
            <span className="text-[0.35em] text-slate-400 ml-3">LB</span>
          </p>
          <p className={`${T.medio} text-slate-400 font-mono tabular-nums mt-1`}>{totalKg.toFixed(2)} kg</p>
        </div>

        <div className="grid grid-rows-2 gap-3">
          <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-blue-500 rounded-xl px-5 flex items-center justify-between gap-3">
            <p className={`${T.medio} text-blue-300 font-semibold uppercase tracking-widest`}>Descabezado</p>
            <p className={`${T.tarjeta} font-mono font-bold tabular-nums text-blue-400 leading-none`}>
              {lb(kgDe(AREA_DESCABEZADO)).toFixed(1)}<span className="text-[0.4em] text-slate-400 ml-2">lb</span>
            </p>
          </div>
          <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-emerald-500 rounded-xl px-5 flex items-center justify-between gap-3">
            <p className={`${T.medio} text-emerald-300 font-semibold uppercase tracking-widest`}>Pelado y Devenado</p>
            <p className={`${T.tarjeta} font-mono font-bold tabular-nums text-emerald-400 leading-none`}>
              {lb(kgDe(AREA_PELADO)).toFixed(1)}<span className="text-[0.4em] text-slate-400 ml-2">lb</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-0 bg-slate-900 border border-slate-800 rounded-xl overflow-hidden flex flex-col">
        ...
        <p className={`${T.medio} px-5 py-1 border-t border-slate-800 ${anteriores > 0 ? "text-slate-400" : "invisible"}`}>
          {anteriores > 0
...
function SinPesadas({ datos }) {
  ...
  return (
    <div className="flex-1 min-h-0 flex flex-col gap-3">
      ...
      <div className="flex-1 min-h-0 bg-slate-900 border border-slate-800 rounded-xl flex flex-col items-center justify-center text-center px-8">
        <svg className="w-[clamp(2.5rem,9vh,5rem)] h-[clamp(2.5rem,9vh,5rem)] text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
            d="M12 3v18M5 7h14M7 7l-3 7a3 3 0 006 0L7 7zm10 0l-3 7a3 3 0 006 0l-3-7z" />
        </svg>
        {noEsDestajo ? (
          <>
            <p className={`${T.nombre} font-bold text-white mt-4`}>TU ÁREA NO TRABAJA POR DESTAJO</p>
            <p className={`${T.chico} text-slate-500 mt-2`}>
          ...
          <>
            <p className={`${T.nombre} font-bold text-white mt-4`}>AÚN NO TIENES PESADAS REGISTRADAS HOY</p>
            <p className={`${T.chico} text-slate-500 mt-2`}>Tu primera pesada aparecerá aquí en cuanto la registre el pesador</p>
            {empleado.HoraEntradaArea && (
              <p className={`${T.chico} text-slate-400 mt-4 font-mono`}>Entraste al área a las {empleado.HoraEntradaArea}</p>
            )}
          </>
        )}
        {ayer.Kilos > 0 && (
          <p className={`${T.chico} text-slate-500 mt-6`}>
            Ayer procesaste <span className="text-slate-300 font-mono tabular-nums">{lb(ayer.Kilos).toFixed(1)} lb</span>
          </p>
        )}
      </div>
    </div>
...
function CampoEscaneo({ inputRef, valor, onChange, onKeyDown, grande }) {
  const clases = grande
    ? `${T.campo} w-[clamp(11rem,26vw,20rem)] py-2 border-2`
    : `${T.medio} w-44 py-1 border`;
  return (
    <input
      ref={inputRef}
      value={valor}
      ...
      autoFocus
      autoComplete="off"
      className={`${clases} bg-slate-900 border-slate-700 rounded-lg px-3 text-center font-mono text-white tracking-widest uppercase focus:outline-none focus:border-emerald-500`}
    />
  );
}
...
function Reposo({ hora, fecha, campo, cargando }) {
  return (
    <div className="flex-1 min-h-0 flex flex-col items-center justify-center text-center">
      <p className="font-mono font-extrabold text-white tabular-nums leading-none text-[clamp(3rem,19vh,10rem)]">{hora.slice(0, 5)}</p>
      <p className={`${T.medio} text-slate-500 mt-2`}>{fecha}</p>
      <svg className="w-[clamp(2.25rem,7vh,4rem)] h-[clamp(2.25rem,7vh,4rem)] text-slate-700 mt-[4vh]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
          d="M3 5h18v14H3V5zm4 4h4v4H7V9zm0 6h10M15 9h2" />
      </svg>
      <p className={`${T.nombre} font-bold text-white mt-3 tracking-wide`}>PASE SU CARNET POR EL LECTOR</p>
      <p className={`${T.medio} mt-2 ${cargando ? "text-emerald-400" : "text-slate-500"}`}>
        {cargando ? "Consultando…" : "Para ver sus libras procesadas de hoy"}
      </p>
...
export default function MiProduccionPage() {
  ...
  return (
    <div
      className="h-screen overflow-hidden bg-slate-950 flex flex-col px-5 py-3 gap-3 select-none"
      onClick={() => inputRef.current?.focus()}
    >
      <button
        onClick={e => { e.stopPropagation(); logout(); window.location.hash = ""; }}
        title="Cerrar sesión"
        className="fixed top-1 right-1 z-50 p-2 rounded-full bg-white/5 hover:bg-white/15 text-slate-700 hover:text-white transition"
      >
      ...
      <div className="flex items-baseline justify-between border-b border-slate-800 pb-2 shrink-0">
        <h1 className={`${T.titulo} text-white font-bold tracking-widest uppercase`}>Mi producción de hoy</h1>
        <div className="flex items-baseline gap-5 pr-8">
          <span className={`${T.chico} text-slate-500`}>{fecha}</span>
          <span className={`${T.reloj} text-white font-mono tabular-nums`}>{hora}</span>
        </div>
      </div>
      ...
      {vista?.tipo === "error" && (
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          <p className={`${T.nombre} font-bold text-amber-400`}>{vista.mensaje}</p>
          <p className={`${T.chico} text-slate-500 mt-3`}>Intente de nuevo o avise a su supervisor</p>
        </div>
      )}
      ...
      {vista && (
        <div className="relative flex items-center justify-center border-t border-slate-800 pt-2 shrink-0">
          <div className="flex items-center gap-3">
            <span className={`${T.medio} ${cargando ? "text-emerald-400" : "text-slate-500"}`}>
              {cargando ? "Consultando…" : "Pase otro carnet para consultar"}
            </span>
            {campo(false)}
          </div>
          {restante != null && (
            <span className={`${T.chico} text-slate-600 absolute right-0`}>Volviendo a inicio en {restante} s</span>
          )}
        </div>
      )}
    </div>
